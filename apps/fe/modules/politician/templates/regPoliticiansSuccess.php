...
<?php use_helper('Date', 'Javascript'	
	...
	, 'HeaderLinks');
		...
		if ( sfRouting::getInstance()->getCurrentRouteName() == 'regione' )
			add_link(
				'@regione_new?slug='. $location->getSlug() .'&location_id='. $location->getId(),
				'canonical');
	?>
...
<div id="title">
  ...
  <h1><?php echo 'Regione'."&nbsp;".$location->GetName(); ?></h1> 
</div>
<hr/>

<?php include_component('politician', 'regionPoliticians', array('location_id'=> $sf_params->get('location_id'))) ?>
<br />
<div id="title">
    <em class="rights-elements">
