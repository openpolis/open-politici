...
<?php use_helper('Date', 'Javascript'
	...
	, 'HeaderLinks');
		...
		if ( sfRouting::getInstance()->getCurrentRouteName() == 'comune' )
			add_link(
				'@comune_new?slug='. $location->getSlug() .'&location_id='. $location->getId(),
				'canonical');
	?>
...
<em class="rights-elements">    
  ...
  <h1><?php echo __('Comune di')."&nbsp;".$location->GetName(); ?>&nbsp;(<?php echo link_to($prov->GetName(),'@provincia_new?location_id='.$prov->GetId().'&slug='.$prov->getSlug()); ?>&nbsp;-&nbsp;<?php echo link_to($region->GetName(),'@regione_new?location_id='.$region->GetId().'&slug='.$region->getSlug()); ?>)</h1>
</div>
<hr/>

<?php include_component('politician', 'municipalPoliticians', array('location_id'=> $sf_params->get('location_id'))) ?>
<br />
<div id="title">
<em class="rights-elements">
