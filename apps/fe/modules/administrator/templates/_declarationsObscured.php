...
<?php if(count($obscured_contents)): ?>
  ...
  <table cellspacing="0" cellpadding="0" border="0">
    ...
    <?php foreach ($obscured_contents as $obscured_content): ?>
      <?php 
        $class = $obscured_content->getOpOpenContent()->getOpContent()->getOpClass();
      ...
      <tr>
        <td><?php 
			echo link_to($content->getOpPolitician()->getLastName(),'@politico_new?content_id='.$content->getOpPolitician()->getContentId(). '&slug='.$content->getOpPolitician()->getSlug());
			//echo link_to($content->getOpPolitician()->getLastName(),'politician/page?content_id='.$content->getOpPolitician()->getContentId());?></td> 
        <td><?php echo link_to($content->getTitle(), 
                               "@dichiarazione_new?" . $content->getSlugUrl()) ?></td>
        <td>
        ...
        <?php if ($sf_user->hasCredential('administrator')): ?>
          <td>
            <?php echo link_to_remote('ripristina', 
                                      array('update' => 'obscured_declarations',
...
<?php else: ?>
  <div>Non ci sono dichiarazioni oscurate</div>
<?php endif; ?>  