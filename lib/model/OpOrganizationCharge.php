...
require_once 'lib/model/om/BaseOpOrganizationCharge.php';


/**
 * Skeleton subclass for representing a row from the 'op_organization_charge' table.
 *
 * 
 *
 ...
 * @package model
 */	
class OpOrganizationCharge extends BaseOpOrganizationCharge {
	...
	public function save($con = null)
	{
		...
		try {
			...
			if ($this->isNew()){
				...
				$cc = $c->getOpContent();
				$cc->setOpTable(OpOrganizationChargePeer::TABLE_NAME);
				$cc->setOpClass("OpOrganizationCharge");
	...
	public function getOrganizationName($con = null)
	{
		$c_affected_rows = 0;
		...
		$cc=OpOrganizationPeer::RetrieveByPk($this->getOrganizationId());
		if($cc)
		  return $cc->getName();
	}
	
	public function getOrganizationUrl($con = null)
	{
		$c_affected_rows = 0;
