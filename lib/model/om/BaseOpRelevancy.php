...
abstract class BaseOpRelevancy extends BaseObject  implements Persistent {
	...
	public function setContentId($v)
	{
		...
		if ($this->content_id !== $v) {
			$this->content_id = $v;
			$this->modifiedColumns[] = OpRelevancyPeer::CONTENT_ID;
		}
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = OpRelevancyPeer::USER_ID;
		}
	...
	public function setScore($v)
	{
		...
		if ($this->score !== $v || $v === 0) {
			$this->score = $v;
			$this->modifiedColumns[] = OpRelevancyPeer::SCORE;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ($this->created_at !== $ts) {
			$this->created_at = $ts;
			$this->modifiedColumns[] = OpRelevancyPeer::CREATED_AT;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating OpRelevancy object", $e);
		}
	}
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			OpRelevancyPeer::doDelete($this, $con);
	...
	{
    if ($this->isNew() && !$this->isColumnModified(OpRelevancyPeer::CREATED_AT))
    {
		...
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aOpUser !== null) {
				...
				$this->setOpUser($this->aOpUser);
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = OpRelevancyPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setNew(false);
				} else {
					$affectedRows += OpRelevancyPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = OpRelevancyPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = OpRelevancyPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = OpRelevancyPeer::getFieldNames($keyType);
		$result = array(
	...
	{
		$criteria = new Criteria(OpRelevancyPeer::DATABASE_NAME);

		if ($this->isColumnModified(OpRelevancyPeer::CONTENT_ID)) $criteria->add(OpRelevancyPeer::CONTENT_ID, $this->content_id);
		if ($this->isColumnModified(OpRelevancyPeer::USER_ID)) $criteria->add(OpRelevancyPeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(OpRelevancyPeer::SCORE)) $criteria->add(OpRelevancyPeer::SCORE, $this->score);
		if ($this->isColumnModified(OpRelevancyPeer::CREATED_AT)) $criteria->add(OpRelevancyPeer::CREATED_AT, $this->created_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(OpRelevancyPeer::CONTENT_ID, $this->content_id);
		$criteria->add(OpRelevancyPeer::USER_ID, $this->user_id);
