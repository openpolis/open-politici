<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdt="http://www.w3.org/2005/xpath-datatypes" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:param name="totale_senatori"/>
	<xsl:template match="/">
	<xsl:variable name="XML1" select="document($XML1)"/>
<body style="font-family:Arial, Helvetica, sans-serif; font-size:10px">
<div style="border:1px solid #9a9797;width:{$dimensione}; background-image:url(http://www.openpolis.it/images/widget/bg4.png); background-repeat:repeat-x; background-position:left bottom;background-color:#000000">
...
<xsl:for-each select="INDICE_ATTIVITA">
	...
	<div style="padding:6px 3px 6px 3px; font-weight:bold; background-color:#c9c7c8;">
	<span style="font-size:11px;"><xsl:value-of select="@nome"/></span><br />
	<span>Indice di attivita'</span>
	</div>
	<div>
	<div style="margin:10px;padding:0 0 8px 0;border-bottom:1px dotted #ffffff;font-weight:bold;">
	<span style="display:block;padding:0 0 5px 0;color:#ffffff">Indice: <span style="background-color:#10952a;color:#ffffff;padding:3px;"><xsl:value-of select="INDICE"/></span> (min 0 - max 10) </span>
	<span style="display:block;padding:0 0 5px 0;color:#ffffff;font-size:13px;"><xsl:value-of select="POSIZIONE"/>° su <xsl:value-of select="$totale_senatori"/></span>
	<span>
	<img>
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="font-weight:bold;color:#ffffff;margin:0 10px 10px 10px;font-size:10px;">
<xsl:for-each select="INDICE_ATTIVITA/SENATORE/node()[contains(.,'a')]">
<tr>
...
font-weight:bold;color:#ffffff;margin:0 10px 10px 10px;font-size:10px;
</xsl:attribute>
<xsl:attribute name="style">
font-weight:bold;color:#ffffff;margin:0 10px 10px 10px;font-size:10px;
</xsl:attribute>
